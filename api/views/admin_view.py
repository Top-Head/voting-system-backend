...
from api.services.cloudinary import (
    upload_to_cloudinary_projects,
    upload_to_cloudinary_members,
    upload_to_cloudinary_stand,
)
...
class ProjectAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        ...
        subcategory_id = self.forwarded.get("subcategory") or self.request.GET.get(
            "subcategory"
        )
        if subcategory_id:
...
def close_activity(request, activity_id):
    ...
    if activity.finished == False:
        ...
        activity.save()
        return Response(
            {"message": "Activity closed successfully"}, status=status.HTTP_200_OK
        )
        ...
        activity.save()
        return Response(
            {"message": "Activity opened sucessfullt"}, status=status.HTTP_200_OK
        )
...
def delete_activity(request, activity_id):
    ...
    return Response(
        {"message": "Activity deleted with sucess"}, status=status.HTTP_200_OK
    )
...
def delete_category(request, category_id):
    ...
    return Response(
        {"message": "Category deleted with sucess"}, status=status.HTTP_200_OK
    )
...
def delete_project(request, project_id):
    ...
    return Response(
        {"message": "Project deleted with sucess"}, status=status.HTTP_200_OK
    )
